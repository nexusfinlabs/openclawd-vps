...
class ControlLog(Base):
    """Audit log for every WhatsApp interaction through oc_control."""
...
class Page(Base):
    """Read-only mirror of the crawler's pages table (for count queries)."""
    __tablename__ = "pages"
...
class ExportCheckpoint(Base):
    """Read-only mirror of export_checkpoints."""
